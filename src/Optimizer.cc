...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    int Optimizer::PoseOptimization(Frame *pFrame)
    {
        ...
        g2o::SparseOptimizer optimizer;
        // Create linear solver
        g2o::BlockSolver_6_3::LinearSolverType* linearSolver_raw = new g2o::LinearSolverDense<g2o::BlockSolver_6_3::PoseMatrixType>();
        std::unique_ptr<g2o::BlockSolver_6_3::LinearSolverType> linearSolver(linearSolver_raw);
        // Create block solver
        g2o::BlockSolver_6_3 * solver_ptr = new g2o::BlockSolver_6_3(std::move(linearSolver));
        // Create optimization algorithm
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    {
//    std::cout << "[TEST] LocalBundleAdjustment1" << std::endl;

        // Local KeyFrames: First Breath Search from Current Keyframe
        list<KeyFrame*> lLocalKeyFrames;

        const auto pkfID = pKF->mnId;  // Cache the value to avoid repeated dereferencing
        lLocalKeyFrames.push_back(pKF);
        pKF->mnBALocalForKF = pkfID;
        Map* pCurrentMap = pKF->GetMap();

        // get covisible keyframes from current frame
        const vector<KeyFrame*> vNeighKFs = pKF->GetVectorCovisibleKeyFrames();

        size_t iend = vNeighKFs.size();

        for(int i=0; i<iend; i++)
        {
            KeyFrame* pKFi = vNeighKFs[i];
            pKFi->mnBALocalForKF = pkfID;
        ...
        for(auto& pMP : lLocalMapPoints)
        {
            ...
            for(auto& observation : observations)
            {
                ...
                if(pKFi->mnBALocalForKF!=pkfID && pKFi->mnBAFixedForKF!=pkfID )
                {
                    pKFi->mnBAFixedForKF=pkfID;
                    if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
...
// Create optimization algorithm
        ...
        auto pMapInitKFid = pMap->GetInitKFid();

        for(auto& pKFi : lLocalKeyFrames)
        {
            auto pKFimnId = pKFi->mnId;
            g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
            Sophus::SE3<float> Tcw = pKFi->GetPose();
            vSE3->setEstimate(g2o::SE3Quat(Tcw.unit_quaternion().cast<double>(), Tcw.translation().cast<double>()));
            vSE3->setId(pKFimnId);
            vSE3->setFixed(pKFimnId==pMapInitKFid);
            optimizer.addVertex(vSE3);
            if(pKFimnId > maxKFid)
                maxKFid=pKFimnId;
            // DEBUG LBA
            pCurrentMap->msOptKFs.insert(pKFimnId);
        }
        ...
        const float thHuberMono = sqrt(5.991); // orig

        const float thHuberStereo = sqrt(7.815); // orig
        ...
        for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
        {
            ...
            for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
            {
                ...
                {
                    const int leftIndex = get<0>(mit->second);
//                const int rightIndex = get<1>(mit->second); // unused

//                std::cout << leftIndex << std::endl;
//                std::cout << rightIndex << std::endl;

                    // std::cout << pKFI->mvuRight(leftIndex) << std::endl;
                    // Monocular observation
...
//                        g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
        ...
        for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), kend = lLocalKeyFrames.end(); lit!=kend; lit++)
        {
            KeyFrame* pKFi = *lit;
        ...
        //Points
        for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), mend = lLocalMapPoints.end(); lit!=mend; lit++)
        {
            MapPoint* pMP = *lit;
            g2o::VertexPointXYZ* vPoint = static_cast<g2o::VertexPointXYZ*>(optimizer.vertex(pMP->mnId+maxKFid+1));
            pMP->SetWorldPos(vPoint->estimate().cast<float>());
...
// Create optimization algorithm
        ...
        for(map<KeyFrame *, set<KeyFrame *> >::const_iterator mit = LoopConnections.begin(); mit!=mend; mit++)
        {
            KeyFrame* pKF = mit->first;
